...
class _ProgressScreenState extends State<ProgressScreen> {
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: _getWeekDays().map((day) => SizedBox(width: 40, child: Text(day, textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12)))).toList(),
            ),
            ...
            child: Column(
              ...
              children: [
                ...
                Center(
                  child: Stack(
                    ...
                    children: [
                      ...
                      Column(
                        children: [
                          Text('${_completionPercentage.toStringAsFixed(0)}%', style: Theme.of(context).textTheme.headlineMedium?.copyWith(fontWeight: FontWeight.bold)),
                          const Text('Complete'),
                        ],
                      ),
                    ],
                ...
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceAround,
                  children: [
                    _buildStatItem('Total', '${stats?.totalAssessments ?? 0}', Icons.assignment),
                    _buildStatItem('Points', '${stats?.totalPoints ?? 0}', Icons.stars),
                    _buildStatItem('Level', '${stats?.level ?? 1}', Icons.leaderboard),
                  ],
                ),
              ],
            ),
        ...
        return Card(
          child: Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Streaks', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                const SizedBox(height: 16),
                Row(
                  children: [
                    _buildStreakCard('Current Streak', currentStreak, Icons.local_fire_department, Colors.orange, currentStreak > 0),
                    const SizedBox(width: 16),
                    _buildStreakCard('Best Streak', longestStreak, Icons.emoji_events, Colors.purple, true),
                  ],
                ),
                const SizedBox(height: 16),
  ...
  Widget _buildStreakCard(String label, int value, IconData icon, Color color, bool highlight) {
    return Expanded(
      child: Container(
        padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(
          color: highlight ? color.withOpacity(0.1) : Colors.grey.shade100,
          borderRadius: BorderRadius.circular(12),
        ),
        child: Column(
          children: [
            Icon(icon, size: 40, color: highlight ? color : Colors.grey),
            const SizedBox(height: 8),
  ...
  Widget _buildStreakMilestones(int currentStreak) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Streak Milestones', style: TextStyle(fontWeight: FontWeight.w500)),
        const SizedBox(height: 8),
        Row(
          mainAxisAlignment: MainAxisAlignment.spaceAround,
              ...
              children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    shape: BoxShape.circle,
                    border: Border.all(color: isAchieved ? Colors.orange : Colors.grey, width: 2),
                  ),
                  child: Center(
                    child: isAchieved
                        ? const Icon(Icons.check, color: Colors.orange, size: 20)
                        : Text('$milestone', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey[600])),
                  ),
                ),
                const SizedBox(height: 4),
                Text('$milestone days', style: TextStyle(fontSize: 10, color: isAchieved ? Colors.orange : Colors.grey)),
              ],
            );
          }).toList(),
        ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 4),
                        Text('$earned of $total earned', style: TextStyle(color: Colors.grey[600])),
                        const SizedBox(height: 8),
                        LinearProgressIndicator(value: earned / total, backgroundColor: Colors.grey[300], color: Colors.amber),
                      ],
                    ),
                  ),
  ...
  // HELPERS
  List<String> _getWeekDays() => ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
